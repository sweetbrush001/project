...
import { ArrowLeft, Bus, Clock, MapPin, TriangleAlert as AlertTriangle, Star, ChevronDown, ChevronUp } from 'lucide-react-native';
import { COLORS, FONT, SIZES, SHADOWS } from '../constants/theme';
import { searchBusSchedulesEnhanced, type BusSchedule } from '../services/busService';
import Animated, { 
  FadeIn, 
  FadeOut,
  SlideInRight,
    ...
    const fetchSchedules = async () => {
      ...
      try {
        if (!from || !to) {
          ...
          return;
        }        // Search for bus schedules using enhanced Firestore search with intermediate stops
        const results = await searchBusSchedulesEnhanced(from, to);
          ...
          const toVariations = [to, `${to} Fort`, to.replace(' Fort', '')];
            ...
            for (const toVar of toVariations) {
              if (fromVar !== from || toVar !== to) {
                const altResults = await searchBusSchedulesEnhanced(fromVar, toVar);
          ...
          <View style={styles.timeSection}>
            ...
            <View style={styles.stopsSection}>
              <TouchableOpacity 
                ...
                onPress={() => setExpandedStops(prev => ({
                  ...prev,
                  [item.trip_id_raw]: !prev[item.trip_id_raw]
                }))}
              >
                ...
                {expandedStops[item.trip_id_raw] ? (
              ...
              {expandedStops[item.trip_id_raw] && (
                <Animated.View entering={FadeIn} style={styles.stopsDetails}>
                  {item.stops.slice(0, 8).map((stop, index) => (
                    <View key={index} style={styles.stopItem}>
                      <View style={styles.stopDot} />
                      <View style={styles.stopInfo}>
                        <Text style={styles.stopName}>{stop.name}</Text>
                        {stop.arrival_time && (
                          <Text style={styles.stopTime}>{formatTime(stop.arrival_time)}</Text>
                        )}
                      </View>
                    </View>
                  ))}
                  {item.stops.length > 8 && (
                    <Text style={styles.moreStopsText}>
                      +{item.stops.length - 8} more stops
                    </Text>
                  )}
                </Animated.View>
              )}
